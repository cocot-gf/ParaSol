...
プライマリはメインCPUから、セカンダリはPCなどをつないでAIの重みデータを動的に更新するのが当初の想定です。
...
# 5 開発コンセプト
Solist-AIの機能が購入者限定などではなく、誰でも簡単に使えるようにする事がこのソフトの最大のコンセプトです。ライセンスに同意いただけるなら、商用を含めて自由に利用していただいて構いません。

* ArduinoやM5Stackなどの外部デバイスでSolist-AIの機能を全部使いたい。
* AIモデルの学習データを自由に読み書きできるようにしたい。
* 開発言語・開発プラットフォームに縛られたくない。
* 高速かつデータ誤りが起きないように通信したい。
* 省電力やCPUが持っている機能をフルに活用してみたい。
* ロームのマイコン内部のプログラム開発は今後一切不要にしたい。
